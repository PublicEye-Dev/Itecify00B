 ...
 */
export async function fetchWorkspaceSnapshotUpdate(workspaceId: string): Promise<Uint8Array | null> {
  const base = apiBase();
  if (!base) return null;
...
export async function persistWorkspaceSnapshot(workspaceId: string, update: Uint8Array): Promise<void> {
  const base = apiBase();
  if (!base) return;
  const url = `${base}/workspaces/${encodeURIComponent(workspaceId)}/snapshot`;
  try {
